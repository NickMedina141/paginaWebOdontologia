...
function selectDay(dateString) {
  ...
  const dayData = calendarData[dateString] || { status: 'available', hours: ['08:00', '09:00', '10:00', '11:00','12:00', '14:00', '15:00', '16:00', '17:00'] };
...
//   });
    ...
    hourCheckboxes.forEach(checkbox => {
        checkbox.addEventListener("change", ()=> {
            const checked = Array.from(hourCheckboxes).filter(c => c.checked);
            if(checked.length > 2){
                checkbox.checked = false;
                showNotification("Solo puede seleccionar una hora de inicio y hora de fin maximo","error");
            }
        });
    // checkbox.checked = dayData.hours.includes(checkbox.value);
  });
...
function saveDayConfig() {
  ...
  //   hora_inicio: status === "available" ? hours[0] : null,
  //   hora_fin: status === "available" ? hours[1] : null,
  //   estado: estadoNum
  // };
  const dataToSend = {
    fecha: selectedDate,
    dia_semana: dia_semana,
    hora_inicio: status === "available" ? hours[0] : "00:00:00",
    hora_fin: status === "available" ? hours[1] : "00:00:00",
    estado: estadoNum
  };
